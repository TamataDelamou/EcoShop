...
| Bannière IA « élèves à risque » consolidée pour la direction | `dashboard_screen.dart _AiBanner` | **Backend prêt (M16, 2026-09-11)** — `risque_reussite` matérialisé/exposé par élève (voir M6 ci-dessus) ; **l'écran d'agrégation (compteur direction) reste à construire**, sous-livrable M16 suivant | Moyen | En cours — sous-livrable suivant de M16 (bannière dashboard directeur) |
...
| Bannière IA « élèves à risque » | `lib/features/dashboard/screens/dashboard_screen.dart` (`_AiBanner`), `TABLEAU_BORD_DIRECTEUR.md` §3bis | Backend prêt (M16) : `statistiques_agregats.risque_reussite`, `risque_reussite_actuel` (migration `20260906001504`) ; écran d'agrégation direction encore à construire (sous-livrable M16 suivant). RPC `risque_classe` (migration M10/M11 §7.3) reste l'agrégat par classe, séparé |
