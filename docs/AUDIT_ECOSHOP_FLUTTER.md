...
| Création et validation d'un nouvel établissement (fondateur, hors invitation à rejoindre un établissement existant) | `DemandeEtablissementModel`, `OnboardingService.creerDemandeEtablissement`/`traiterDemandeEtablissement`, `validation_demandes_etablissement_screen.dart` | **Absent** — aucune RPC, Edge Function ni écran côté cible ; `supabase/migrations/20260904000000_core_schema.sql` lignes 264-266 délègue explicitement cette écriture à « un parcours d'onboarding/validation (M1/M2) » qui n'existe pas pour ce cas précis (seul `demandes_adhesion`/`accepter_invitation` y est livré, et ne couvre que le rattachement à un établissement **existant**, pas la création d'un nouveau). Contredit donc partiellement la ligne « Parcours d'entrée Enseignant/Direction/Vendeur/Fondateur réseau » ci-dessus, dont la recommandation « RPC déjà livrée, seul le client manque » ne s'applique pas à ce sous-cas. Un établissement ne peut aujourd'hui être créé que par insertion manuelle (seed/SQL direct) | Élevé | Différer — arbitrage de priorité explicitement reporté par le porteur de projet (2026-09-17), probable candidat prioritaire (aucun onboarding de client réel possible sans lui), décision à tête reposée en début de prochaine session |
...
| Création et validation d'un nouvel établissement (fondateur) | `lib/models/demande_etablissement_model.dart`, `lib/services/onboarding_service.dart` (`creerDemandeEtablissement`, `traiterDemandeEtablissement`, `streamDemandesEtablissementEnAttente`), `lib/features/gsg_backoffice/screens/validation_demandes_etablissement_screen.dart` | absent — aucune table/RPC/Edge Function/écran équivalent ; `supabase/migrations/20260904000000_core_schema.sql` lignes 264-266 renvoie vers un onboarding M1/M2 qui ne couvre pas ce cas (seul le rattachement à un établissement existant, `demandes_adhesion`, y est livré) |
