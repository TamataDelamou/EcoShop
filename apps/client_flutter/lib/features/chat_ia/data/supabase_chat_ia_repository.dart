...
class SupabaseChatIaRepository implements ChatIaRepository {
  ...
  Future<ReponseChatIa> envoyerMessage({
    ...
    return _executer(() async {
      final reponse = await _client.functions.invoke('envoyer_message_ia', body: {
  ...
  Future<ReponseAnalyseRisque> demarrerAnalyseRisque({
    ...
    return _executer(() async {
      final reponse = await _client.functions.invoke('demarrer_analyse_risque_echec', body: {
      ...
      return ReponseAnalyseRisque(
        ...
        messageDeclencheur: "Analyse du risque d'échec pour $cibleLabel. "
            "Effectif : $effectif. "
  ...
  Future<MessageChatIa> enregistrerMessage({
    ...
    return _executer(() async {
      final ligne = await _client
          .from('ai_messages')
          .insert({'conversation_id': conversationId, 'sender': sender, 'content': content})
          .select()
          .single();
